...
run_fuzzer()
{
    ...
    echo "ASAN_OPTIONS=strict_string_checks=1:detect_stack_use_after_return=1:check_initialization_order=1:strict_init_order=1:log_path=$directory/logs/asan$BUILD_CONFIG.log:halt_on_error=0 UBSAN_OPTIONS=halt_on_error=0 LSAN_OPTIONS=detect_leaks=0 $directory/run_$BUILD_CONFIG/sbin/ns-slapd -D $directory/run_$BUILD_CONFIG/etc/dirsrv/slapd-test-instance -i $directory/run_$BUILD_CONFIG/run/dirsrv/slapd-test-instance.pid $config_flags $FUZZ  >> $directory/logs/error$BUILD_CONFIG 2>>$directory/logs/error$BUILD_CONFIG &"
    ...
    echo "ASAN_OPTIONS=strict_string_checks=1:detect_stack_use_after_return=1:check_initialization_order=1:strict_init_order=1:log_path=$directory/logs/asan$BUILD_CONFIG.log:halt_on_error=0 UBSAN_OPTIONS=halt_on_error=0 LSAN_OPTIONS=detect_leaks=0 $directory/run_$BUILD_CONFIG/sbin/ns-slapd -D $directory/run_$BUILD_CONFIG/etc/dirsrv/slapd-test-instance -i $directory/run_$BUILD_CONFIG/run/dirsrv/slapd-test-instance.pid $config_flags $FUZZ &"
    ASAN_OPTIONS=strict_string_checks=1:detect_stack_use_after_return=1:check_initialization_order=1:strict_init_order=1:log_path=$directory/logs/asan$BUILD_CONFIG.log:halt_on_error=0 UBSAN_OPTIONS=halt_on_error=0  LSAN_OPTIONS=detect_leaks=0 $directory/run_$BUILD_CONFIG/sbin/ns-slapd -D $directory/run_$BUILD_CONFIG/etc/dirsrv/slapd-test-instance -i $directory/run_$BUILD_CONFIG/run/dirsrv/slapd-test-instance.pid $config_flags $FUZZ &
    fi
